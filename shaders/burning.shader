...
uniform float Gradient_Adjust = 0.35;
uniform float Dissolve_Value = 2.0;
uniform bool Animated = true;
uniform bool Apply_to_Channel = true;
...
float4 mainImage(VertData v_in) : TARGET
{
	...
	if (!Animated)
		cycle = 1;
	float d = clamp(((Dissolve_Value * cycle + perlin) ) - 1.0, 0.02, 0.99);
