...
uniform float Border_Offset<
    ...
    float step = 0.01;
> =1.01;
uniform float4 Border_Color = {0.6,.95,1.0,1.0};
uniform float4 Glass_Color = {0.6,.95,1.0,1.0};
...
float4 mainImage(VertData v_in) : TARGET
{
	...
	int xSubPixel = int(mod((v_in.uv.x * uv_size.x) , float(clamp(xSize,1,500))));
	int ySubPixel = int(mod((v_in.uv.y * uv_size.y) , float(clamp(ySize,1,500))));
